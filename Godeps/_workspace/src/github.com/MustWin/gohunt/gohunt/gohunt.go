...
const (
	...
	collectionUrl        = base + "/v1/collections"
...
type multiCollectionResponse struct {
...
func (c *Client) GetCollection(id int) (Collection, error) {
	return c.submitSingleCollectionRequest(collectionUrl +"/"+strconv.Itoa(id), nil)
}

func (c *Client) GetCollections() ([]Collection, error) {
	return c.submitCollectionRequest(collectionUrl, nil)
}

func (c *Client) submitSingleCollectionRequest(url string, values *url.Values) (Collection, error) {
	collectionmap := &singleCollectionResponse{}
	err := c.submitJsonRequest(url, nil, collectionmap)
	if err != nil {
		return Collection{}, err
	}
	return collectionmap.Collection, nil
}

func (c *Client) submitCollectionRequest(url string, values *url.Values) ([]Collection, error) {
	collectionmap := &multiCollectionResponse{}
	err := c.submitJsonRequest(url, values, collectionmap)
	if err != nil {
		return nil, err
	}
	return collectionmap.Collections, nil
}
