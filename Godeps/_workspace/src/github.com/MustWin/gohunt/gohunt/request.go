...
func (r Request) getAuthResponse(auth string) (*bytes.Buffer, error) {
	...
	if resp.StatusCode != http.StatusOK {
        if resp.StatusCode == 429 {
            ...
            rateLimitHeader := resp.Header["X-Rate-Limit-Reset"]
            limitTime, err2 := strconv.Atoi(rateLimitHeader[0])
            emptyResp := bytes.NewBufferString("")
            if err2 != nil  {
                return emptyResp, errors.New(err.Error() + "|" + err2.Error())
            }
		    return emptyResp, ResponseError{err, limitTime}
        }
